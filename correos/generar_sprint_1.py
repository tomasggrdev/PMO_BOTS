...
CREDENTIALS_PATH = BASE_PATH + "/credenciales.json"
...
def read_sprint(links, numeroSprint, driver):
    dest_filename = 'sprint_1.xlsx'
    frame = pd.DataFrame(
        {"Codigo": [], "Codigo_Id": [], "Nombre_Tripulante": [], "Estado": [], "Nota_Final": [], "Grupo": [],"Sprint": []}
    )

    contador_lineas = 0
    for i in range(59):
        contador_tripulantes = 1
        ...
        while flag == True:
            try:
                codigo = driver.find_element(By.XPATH,f"/html/body/div[1]/div[2]/div[5]/div[3]/div[2]/div/div/div/div/div/div[3]/div[{a}]/table/tbody/tr[{contador_tripulantes}]/td[4]").text
                codigo_id = driver.find_element(By.XPATH,f"/html/body/div[1]/div[2]/div[5]/div[3]/div[2]/div/div/div/div/div/div[3]/div[{a}]/table/tbody/tr[{contador_tripulantes}]/td[5]").text
                nombre = driver.find_element(By.XPATH,f"/html/body/div[1]/div[2]/div[5]/div[3]/div[2]/div/div/div/div/div/div[3]/div[{a}]/table/tbody/tr[{contador_tripulantes}]/td[3]").text
                estado = driver.find_elements(By.XPATH,f"/html/body/div[1]/div[2]/div[5]/div[3]/div[2]/div/div/div/div/div/div[3]/div[{a}]/table/tbody/tr[{contador_tripulantes}]/td[6]/div")
                calificacion_final = driver.find_element(By.XPATH,f"/html/body/div[1]/div[2]/div[5]/div[3]/div[2]/div/div/div/div/div/div[3]/div[{a}]/table/tbody/tr[{contador_tripulantes}]/td[15]").text
                #ws1 = wb.active
                ...
                for j in range(len(estado)):
                    ...
                    if j==0:
                        frame.loc[contador_lineas] = [codigo, codigo_id, nombre, estado[j].text, calificacion_final, f"O{i + 1}", f"s{numeroSprint}"]
                        #frame.append([codigo, codigo_id, nombre, estado[j].text, calificacion_final, f"O{i + 1}", f"s{numeroSprint}"])
                        contador_lineas = contador_lineas + 1
                    else:
                        frame.loc[contador_lineas] = ["", "", "", estado[j].text, "", "", ""]
                        contador_lineas = contador_lineas + 1
                ...
                flag = False
    frame.to_excel(f"sprint_{numeroSprint}.xlsx",sheet_name=f"sprint_{numeroSprint}",index=False)



read_sprint(links_sprint_1,1,driver)
